...
#------------------------------------------------------------------------------
#
# Reduce Dimensionality with LSA (Train)
#
#------------------------------------------------------------------------------
...
classifier = SVC(class_weight={0:0.75, 1:0.25});
classifier.fit(X, dataset_train.target);
...
#------------------------------------------------------------------------------
#
# Reduce Dimensionality with LSA (Test)
#
#------------------------------------------------------------------------------

print(">> Reducing Dimensionality with LSA for testing...")
t0 = time()
svd_t = TruncatedSVD(100)
normalizer_t = Normalizer(copy=False)
lsa_t = make_pipeline(svd_t, normalizer_t)

X_t = lsa_t.fit_transform(X_t)

[n_samples, n_features] = X_t.shape

print("   done in %fs" % (time() - t0))

explained_variance = svd_t.explained_variance_ratio_.sum()
print("   Explained variance of the SVD step: {}%".format(
        int(explained_variance * 100)))
